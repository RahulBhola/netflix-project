...
const OpenImageOverlay = ({ isOpen, seriesData, closeOverlay, picture }) => {
    ...
    const fetchEpisodes = async (seasonId) => {
        try {
            const response = await fetch(`http://localhost:3000/auth/getEpisodes/${seasonId}`);
            if (!response.ok) {
        ...
        try {
            const response = await fetch('http://localhost:3000/auth/addToList', {
                ...
                body: JSON.stringify({
                    ...
                    details: seriesData.seriesDetails,
                    cast: seriesData.seriesCast,
                    creators: seriesData.seriesCreators,
                    picture: picture,
                    trailerVideo: seriesData.trailerVideo,
                    trailerVideoPoster: seriesData.trailerVideoPoster,
